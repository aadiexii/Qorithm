...
- `src/app/admin`: Strict admin-only KPI-driven management routes.
- `src/app/dashboard`: Personalized progress and engagement view.
...
- `src/features`: Domain-driven feature slicing (UI, data, server actions).
  - `src/features/dashboard`: Solved-focus data, POTD logic, platform sync logic.
- `src/lib/platforms`: Adapter pattern implementation for syncing with Codeforces and AtCoder.
- `src/lib/problem-url.ts`: Shared logic for building correct external problem URLs.
...
- **API Protection**: Platform sync APIs and data mutations use robust server actions that validate the user session securely.
