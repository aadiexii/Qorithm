...
export async function searchProblemsForMapping(query: string = "") {
  ...
  return db
    ...
    .where(
      query
        ? sql`${problems.title} ILIKE ${search} OR ${problems.source} ILIKE ${search}`
        : undefined,
    )
...
export async function addProblemToSection(
  sectionId: string,
  problemId: string,
) {
  await requireAdmin();
  ...
  try {
    ...
    if (
      ...
      return {
        success: false,
        error: "Problem is already mapped to this section.",
      };
    }
    throw err;
  }
...
export async function removeProblemFromSection(
  sectionId: string,
  problemId: string,
) {
  await requireAdmin();
...
export async function moveProblemOrder(
  ...
  const mappings = await db
    .select({
      problemId: sheetSectionProblems.problemId,
      orderIndex: sheetSectionProblems.orderIndex,
    })
    .from(sheetSectionProblems)
    .where(eq(sheetSectionProblems.sectionId, sectionId))
  ...
  if (direction === "down" && currentIndex === mappings.length - 1)
