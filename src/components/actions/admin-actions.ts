...
import {
  ...
  problemTopics,
  topics,
} from "@/services/Database/schema/problems";
...
export async function adminCreateProblemAction(
  ...
  try {
    ...
    const finalExtContest =
      typeof externalContestId === "number" ? externalContestId : null;
...
export async function adminDeleteProblemAction(
  id: string,
): Promise<ActionState> {
  const session = await requireAdmin();
  if (!session) return { success: false, message: "Admin access required." };
...
export async function bulkImportAction(
  ...
  for (let i = 0; i < dataLines.length; i++) {
    ...
    validRows.push({
      ...
      externalContestId:
        typeof data.externalContestId === "number"
          ? data.externalContestId
          : null,
...
import { count, gte, sql, and, lte } from "drizzle-orm";
...
export async function getAdminAnalytics(): Promise<AdminAnalytics> {
  ...
  // Signups previous 7d
  const [signupsPrev7dRes] = await db
    .select({ value: count() })
    .from(users)
    .where(
      and(
  ...
  // Solves 7d
  const [solves7dRes] = await db
    .select({ value: count() })
    .from(userProblemStates)
    .where(
      and(
        eq(userProblemStates.status, "solved"),
        gte(userProblemStates.updatedAt, sevenDaysAgo),
      ),
    );
  const solves7d = solves7dRes?.value ?? 0;

  // Solves previous 7d
  const [solvesPrev7dRes] = await db
    .select({ value: count() })
    .from(userProblemStates)
    .where(
      and(
