...
} from "@/components/actions/sheet-actions";
...
function StatusBadge({
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-medium ${styles[label]}`}
    >
      {label}
    </span>
  );
...
function SectionCard({
  ...
  return (
    <Link
      ...
      className={`group focus-visible:ring-ring relative flex flex-col gap-4 overflow-hidden rounded-2xl border p-5 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-black/40 focus-visible:ring-2 focus-visible:outline-none ${
        isRecommended
    ...
    >
      {isRecommended && (
        <div className="absolute top-0 right-0 rounded-bl-xl bg-white px-3 py-1 text-xs font-bold tracking-wider text-black uppercase">
          Continue
        </div>
      )}
      ...
      <div className="flex items-start justify-between gap-3 pr-20">
        <div className="space-y-1.5">
          <h3 className="leading-snug font-semibold text-white transition-colors group-hover:text-white/80">
            {section.title}
          </h3>
...
export default async function SheetPage() {
  ...
  const completedCount = sections.filter(
    (s) => s.statusLabel === "Completed",
  ).length;
  const inProgressCount = sections.filter(
    (s) => s.statusLabel === "In Progress",
  ).length;
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-10 px-4 py-10 md:px-8">
      ...
      <div className="space-y-2">
        <div className="text-muted-foreground flex items-center gap-2 text-sm font-medium tracking-widest uppercase">
        ...
        <p className="max-w-2xl text-base text-slate-400">
          Work through each section in order. Problems within sections are
          curated for deliberate progression.
        </p>
      </div>
      ...
      {recommended && (
        <Card className="border-white/15 bg-white/5">
          <CardHeader className="pb-3">
            <CardDescription className="text-muted-foreground text-xs font-medium tracking-widest uppercase">
          ...
          <CardContent className="flex items-center justify-between gap-4">
            <div className="flex-1 space-y-1.5">
              ...
              <p className="text-xs text-slate-400">
                {recommended.progressPercentage}% complete
              </p>
            </div>
            <Link
              href={`/sheet/${recommended.slug}`}
              className="focus-visible:ring-ring inline-flex shrink-0 items-center gap-2 rounded-xl border border-white/15 bg-white px-5 py-2.5 text-sm font-semibold text-black transition hover:bg-white/90 focus-visible:ring-2 focus-visible:outline-none"
            >
      ...
      {sections.length === 0 ? (
        <Card className="border-border/60 bg-card/70">
          <CardContent className="flex flex-col items-center justify-center gap-4 py-16 text-center">
            <div className="border-border/60 bg-card/70 flex h-14 w-14 items-center justify-center rounded-full border">
